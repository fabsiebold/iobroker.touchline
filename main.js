/**
 *
 * touchline adapter
 *
 *
 *  file io-package.json comments:
 *
 *  {
 *      "common": {
 *          "name":         "touchline",                // name has to be set and has to be equal to adapters folder name and main file name excluding extension
 *          "version":      "0.99.0",                   // use "Semantic Versioning"! see http://semver.org/
 *          "title":        "Roth Touchline Adapter",   // Adapter title shown in User Interfaces
 *          "authors":  [                               // Array of authord
 *              "name <fabian.siebold@gmail.com>"
 *          ]
 *          "desc":         "Roth Touchline adapter",   // Adapter description shown in User Interfaces. Can be a language object {de:"...",ru:"..."} or a string
 *          "platform":     "Javascript/Node.js",       // possible values "javascript", "javascript/Node.js" - more coming
 *          "mode":         "schedule",                 // possible values "daemon", "schedule", "subscribe"
 *          "materialize":  true,                       // support of admin3
 *          "schedule":     "0 0/5 * * *"               // cron-style schedule. Only needed if mode=schedule
 *          "loglevel":     "info"                      // Adapters Log Level
 *      },
 *      "native": {                                     // the native object is available via adapter.config in your adapters code - use it for configuration
 *          "ipAddress": "0.0.0.0",
 *      }
 *  }
 *
 */

/* jshint -W097 */// jshint strict:false
/*jslint node: true */
'use strict';
var exec = require('child_process').exec;

var html = "<p>In practice, it is usually a bad idea to modify global variables inside the function scope since it often be the cause of confusion and weird errors that are hard to debug.<br />If you want to modify a global variable via a function, it is recommended to pass it as an argument and reassign the return-value.<br />For example:</p>"


// you have to require the utils module and call adapter function
const utils          = require(__dirname + '/lib/utils'); // Get common adapter utils
const XMLHttpRequest = require(__dirname + '/lib/XMLHttpRequest'); // Get common adapter utils


// you have to call the adapter function and pass a options object
// name has to be set and has to be equal to adapters folder name and main file name excluding extension
// adapter will be restarted automatically every time as the configuration changed, e.g system.adapter.touchline.0
const adapter = new utils.Adapter('touchline');

/*Variable declaration, since ES6 there are let to declare variables. Let has a more clearer definition where 
it is available then var.The variable is available inside a block and it's childs, but not outside. 
You can define the same variable name inside a child without produce a conflict with the variable of the parent block.*/
let pollingInterval;
let waittime = 3000;

let imageDataDevice		= 'data:image/png;base64,';

let numOfRUs = 0;
let iRU = 0;


let property = {
    "Name": {
//        "init":false,
		"type": 'state',
        "common": {
            name: 'Name',
            type: 'string',
            unit: '',
            role: '',
            read: false,
            write: false
        },
        "native": {}
    },
    "RaumTemp": {
//        "init":false,
		"type": 'state',
        "common": {
            name: 'Temperatur',
            type: 'number',
            unit: '°C',
            role: 'value',
            read: true,
            write: false,
            // custom: {
              // 'history.0': {
                // enabled: true,
                // changesOnly: true,
                // debounce: "1000",
                // maxLength: "250",
                // retention: "31536000",
                // changesRelogInterval: 0,
                // changesMinDelta: 0,
                // aliasId: ""
              // }
            // }
        },
        "native": {}
    },
    "SollTemp": {
//        "init":21,
		"type": 'state',
        "common": {
            name: 'Solltemperatur',
            type: 'number',
            unit: '°C',
            role: 'value',
            read: true,
            write: true,
            min: 5,
            max: 30,
            // custom: {
              // 'history.0': {
                // enabled: true,
                // changesOnly: true,
                // debounce: "1000",
                // maxLength: "250",
                // retention: "31536000",
                // changesRelogInterval: 0,
                // changesMinDelta: 0,
                // aliasId: ""
              // }
            // }
        },
        "native": {}
    },
    "OPMode": {
//        "init":0,
		"type": 'state',
        "common": {
            name: 'Mode',
            type: 'string',
            unit: '',
            role: 'value',
            read: true,
            write: true,
            states: '0:Tag;1:Nacht;2:Aus',
            min: 0,
            max: 3,
            // custom: {
              // 'history.0': {
                // enabled: true,
                // changesOnly: true,
                // debounce: "1000",
                // maxLength: "250",
                // retention: "31536000",
                // changesRelogInterval: 0,
                // changesMinDelta: 0,
                // aliasId: ""
              // }
            // }
        },
        "native": {}
    },
    "WeekProg": {
//        "init":false,
		"type": 'state',
        "common": {
            name: 'Wochenprogramm',
            type: 'string',
            unit: '',
            role: '',
            read: true,
            write: true,
            states: '0:Aus;1:Programm I;2:Programm II;3:Programm III',
            min: 0,
            max: 3,
            // custom: {
              // 'history.0': {
                // enabled: true,
                // changesOnly: true,
                // debounce: "1000",
                // maxLength: "250",
                // retention: "31536000",
                // changesRelogInterval: 0,
                // changesMinDelta: 0,
                // aliasId: ""
              // }
            // }
        },
        "native": {}
    },
    "TempSIUnit": {
//        "init":false,
		"type": 'state',
        "common": {
            name: 'Einheit',
            type: 'string',
            unit: '',
            role: 'unit',
            read: false,
            write: false
        },
        "native": {}
    },
    "SollTempMaxVal": {
//        "init":false,
		"type": 'state',
        "common": {
            name: 'maximale Solltemperatur',
            type: 'number',
            unit: '',
            role: 'range',
            read: false,
            write: false
        },
        "native": {}
    },
    "SollTempMinVal": {
//        "init":false,
		"type": 'state',
        "common": {
            name: 'minimale Solltemperatur',
            type: 'number',
            unit: '',
            role: 'range',
            read: false,
            write: false
        },
        "native": {}
    },
    "SollTempStepVal": {
//        "init":false,
		"type": 'state',
        "common": {
            name: 'Solltemperatur Schrittweite',
            type: 'number',
            unit: '',
            role: 'range',
            read: false,
            write: false
        },
        "native": {}
    },
    "OPModeEna": {
//        "init":false,
		"type": 'state',
        "common": {
            name: 'Mode aktive',
            type: 'boolean',
            unit: '',
            role: '',
            read: false,
            write: false
        },
        "native": {}
    },
    "WeekProgEna": {
//        "init":false,
		"type": 'state',
        "common": {
            name: 'Wochenprogramm aktiv',
            type: 'boolean',
            unit: '',
            role: '',
            read: false,
            write: false
        },
        "native": {}
    },
    "ownerKurzID": {
//        "init":false,
		"type": 'state',
        "common": {
            name: 'ownerKurzID',
            type: 'string',
            unit: '',
            role: '',
            read: false,
            write: false
        },
        "native": {}
    },
    "kurzID": {
//        "init":false,
		"type": 'state',
        "common": {
            name: 'Kurz-Id',
            type: 'string',
            unit: '',
            role: '',
            read: false,
            write: false
        },
        "native": {}
    },
    "connection": {
        "init": false,
		"type": 'state',
        "common": {
            name: 'connection established',
            type: 'boolean',
            unit: '',
            role: '',
            read: false,
            write: false
        },
        "native": {}
    },
};



// is called when adapter shuts down - callback has to be called under any circumstances!
adapter.on('unload', function (callback) {
    try {
        adapter.log.info('cleaned everything up...');
		adapter.setState("info.connection", {val: false, ack: true});
        callback();
    } catch (e) {
        callback();
    }
});

// is called if a subscribed object changes
adapter.on('objectChange', function (id, obj) {
    // Warning, obj can be null if it was deleted
    adapter.log.info('objectChange ' + id + ' ' + JSON.stringify(obj));
});

// is called if a subscribed state changes
adapter.on('stateChange', function (id, state) {
    // Warning, state can be null if it was deleted
//    adapter.log.info('stateChange ' + id + ' ' + JSON.stringify(state));

    // you can use the ack flag to detect if it is status (true) or command (false)
    if (state && !state.ack) 
	{
		let value = state.val;
		let addrArray = id.split(".");
		let prop = addrArray.pop();
		let dev  = addrArray.pop();
		let addr = dev + "." + prop;

		// Was wurde geaendert?
		if (prop == "SollTemp")
		{
			// Ist der Wert ausserhalb des gütligen Bereich (5-30)?
			value = Math.min(Math.max(5,value), 30);

			// Der Wert wird in Hunderstelgrad geschrieben
			value = (Math.round(value * 10) * 10);
		}
		else if (prop == "OPMode")
		{
			// Ist der Wert ausserhalb des gütligen Bereich (0-2)?
			value = Math.min(Math.max(0,value), 2);
		}
		else if (prop == "WeekProg")
		{
			// Ist der Wert ausserhalb des gütligen Bereich (0-3)?
			value = Math.min(Math.max(0,value), 3);
		}
		else 
		{
			adapter.log.error('Value \'' + prop + '\' not writable');
			return;
		}
        
        // Schreibe den neuen Wert
		adapter.log.debug("Value of <" + addr + "> changed to <" + value + ">");
		writeValue(addr, value); 
		
		setTimeout(getData, waittime);
	}
});

// Some message was sent to adapter instance over message box. Used by email, pushover, text2speech, ...
adapter.on('message', function (obj) {
    if (typeof obj === 'object' && obj.message) {
        if (obj.command === 'send') {
            // e.g. send email or pushover or whatever
            console.log('send command');

            // Send response in callback if required
            if (obj.callback) adapter.sendTo(obj.from, obj.command, 'Message received', obj.callback);
        }
    }
});

// is called when databases are connected and adapter received configuration.
// start here!
adapter.on('ready', function () {
    adapter.log.info("ROTH-Touchline: START ADAPTER data:", adapter.common);
	adapter.setState("info.connection", {val: false, ack: true});
    main();
	getData();
});



function main() {

    // The adapters config (in the instance object everything under the attribute "native") is accessible via
    // adapter.config:
    adapter.log.info('IP-Addresse:       ' + adapter.config.ipAddress);
    adapter.log.info('Polling-Intervall: ' + adapter.config.pollingInterval);
    adapter.log.info('Waittime:          ' + adapter.config.waittime);

	if (!adapter.config.waittime)
	{
        adapter.log.warn("adapter.config.waittime not set");
		waittime = 3000;
	}
	else if (adapter.config.waittime > 10000)
	{
        adapter.log.warn("waittime to high ==> set to 10000ms");
		waittime = 10000;
	}
	else
	{
		waittime = adapter.config.waittime;
	}
	
	if (!adapter.config.pollingInterval)
	{
        adapter.log.warn("adapter.config.pollingInterval not set");
		adapter.config.pollingInterval = 5;
	}

	if (adapter.config.pollingInterval < 5)
	{
		adapter.config.pollingInterval = 5;
	}
	
	if (!pollingInterval) {
		pollingInterval = setInterval(function () {
			getData();
		}, adapter.config.pollingInterval * 1000);
	}
	
    // in this touchline all states changes inside the adapters namespace are subscribed
    adapter.subscribeStates('*');

/*
    // examples for the checkPassword/checkGroup functions
    adapter.checkPassword('admin', 'iobroker', function (res) {
        console.log('check user admin pw ioboker: ' + res);
    });

    adapter.checkGroup('admin', 'admin', function (res) {
        console.log('check group user admin group admin: ' + res);
    });
*/
}


function getCount() {
	let dataGetCount = "\" <body><version>1.0</version><item_list_size>1</item_list_size><item_list><i><n>totalNumberOfDevices</n></i></item_list></body> \""

	// prepare request
	var xhr = new XMLHttpRequest();
	xhr.open("POST", "http://"+adapter.config.ipAddress+"/cgi-bin/ILRReadValues.cgi", true);
	xhr.setRequestHeader("Content-Type", "text/xml");
	xhr.setRequestHeader("User-Agent", "Roth-Touchline.../1.05");  //Basic Headers needed :)

	xhr.onreadystatechange = function() { // Call a function when the state changes.
		if (this.readyState === this.DONE && this.status === 200) {
			let start = this.responseText.indexOf('<v>')+3;		  
			let end   = this.responseText.indexOf('</v>');
		 
			// Tag <v>x</v> not found
			if (start > end) {				
				numOfRUs = 0;
			}
			else {
				numOfRUs = parseInt(this.responseText.substr(start,(end-start)));
			}

			adapter.log.info("Found " + numOfRUs + " RoomUnits in the device <" + adapter.config.ipAddress + ">");

			// connection to device established
			adapter.setState("info.connection", {val: true, ack: true});

			//	create all elements
			for (let i=0; i<numOfRUs; i++) {
				let ruName = "G"+i;
				// init data of RU-Object 
				let ruData = {
					"init":false,
					"type": 'device',
					"common": {
						name: 'Roomunit ' + ruName,
						role: 'device',
						icon: imageDataDevice
					},
					"native": {}
				};

				adapter.log.info("ROTH-Touchline: create RoomUnit Id=" + ruName);
				adapter.setObject(ruName, ruData);

				for (var prop in property) {
					let propName = ruName + "." + prop;
	                adapter.log.debug("ROTH-Touchline: create Property Id=" + propName + property[prop]);
					adapter.setObject(propName, property[prop]);
				}
			}

			// Start getData
			setTimeout(getData, waittime);
		}
		else if (this.status < 200 && this.status >= 300)  {
			adapter.log.error("Unable to get RU count from <" + adapter.config.ipAddress + ">");
			adapter.log.error(this.statusText, this.responseText);
			adapter.setState("info.connection", {val: false, ack: true});
		}
		
	}
	xhr.send(dataGetCount);
}


// Average Load mit uptime auslesen
function getData() 
{
	let timeout = waittime * 2;

	if (numOfRUs <= 0)
	{
		getCount();
	}


	// All RU received
	if (iRU >= numOfRUs) {
		// nothing to do => wait for next timer event
		iRU = 0;		
	}
	else
	{
		// prepare request
		var xhrRoom = new XMLHttpRequest();
		xhrRoom.open("POST", "http://"+adapter.config.ipAddress+"/cgi-bin/ILRReadValues.cgi", true);
		xhrRoom.setRequestHeader("Content-Type", "text/xml");
		xhrRoom.setRequestHeader("User-Agent", "Roth-Touchline.../1.05");  //Basic Headers needed :)
		let roomData = '<body><version>1.0</version><client>App</client><file_name>Controller</file_name><item_list_size>13</item_list_size><item_list><i><n>G'+iRU+'.kurzID</n></i><i><n>G'+iRU+'.ownerKurzID</n></i><i><n>G'+iRU+'.RaumTemp</n></i><i><n>G'+iRU+'.SollTemp</n></i><i><n>G'+iRU+'.OPMode</n></i><i><n>G'+iRU+'.WeekProg</n></i><i><n>G'+iRU+'.TempSIUnit</n></i><i><n>G'+iRU+'.SollTempMaxVal</n></i><i><n>G'+iRU+'.SollTempMinVal</n></i><i><n>G'+iRU+'.SollTempStepVal</n></i><i><n>G'+iRU+'.OPModeEna</n></i><i><n>G'+iRU+'.WeekProgEna</n></i><i><n>CD.rooms['+iRU+']</n></i></item_list></body>'
		adapter.log.debug("roomData Request " + roomData);

		xhrRoom.onreadystatechange = function() {
			// Call a function when the state changes.
			if (this.readyState === this.DONE && this.status === 200) {
				adapter.log.debug("roomData Response " + this.responseText);
			
				let elements = this.responseText.split('<n>');
				
				for (let element in elements) {
					// pass first element
					if (element == 0)
						continue;
					
					let tagStr = '' + elements[element];
					let valStr = '' + elements[element].split('<v>')[1];

					tagStr = tagStr.substr(0, tagStr.indexOf('<'));
					valStr = valStr.substr(0, valStr.indexOf('<'));
					let prop =  '' + tagStr.split('.')[1];

					adapter.log.debug("Set Property <" + prop + "> of Dev <" + tagStr + "> with value " + valStr);
					
					if ((prop == "RaumTemp") || (prop == "SollTemp") || (prop == "SollTempMaxVal") || (prop == "SollTempMinVal") || (prop == "SollTempStepVal")) {
						adapter.setState(tagStr, {val: valStr/100, ack: true, expire: timeout});
					}
					else if ((prop == "WeekProgEna") || (prop == "OPModeEna")) {
						adapter.setState(tagStr, {val: (valStr==true), ack: true, expire: timeout});
					}
					else if ((prop.substr(0,6) == "rooms[")) {
						adapter.setState("G" + prop.charAt(6) + ".Name",       {val: valStr, ack: true, expire: timeout});
						adapter.setState("G" + prop.charAt(6) + ".connection", {val: true, ack: true, expire: timeout});
					}
					else {
						adapter.setState(tagStr, {val: valStr, ack: true, expire: timeout});
					}
				}
			}
			else if (this.status < 200 && this.status >= 300) {
				adapter.log.error("Error while set <"+addr+"> to <" + value + ">");
				adapter.log.error(this.statusText, this.responseText);
			}

		}
		xhrRoom.send(roomData);

		// Not all RUs received?
		if (++iRU < numOfRUs) {
			setTimeout(getData, waittime);
		}
	}
}


// Schreibe einen neuen Wert zum Touchline
// value=`curl -s -k  -X 'GET' -H 'User-Agent: SpiderControl/1.0 (iniNet-Solutions GmbH)'  "http://$ROTHIP/cgi-bin/writeVal.cgi?$1=$2"`
//                                                                                          http://192.168.10.221/cgi-bin/writeVal.cgi?G0.SollTemp=2000
function writeValue(addr, value) 
{
	var request = new XMLHttpRequest();
	var url = 'http://' + adapter.config.ipAddress + '/cgi-bin/writeVal.cgi?' + addr + '=' + value;

	request.open("GET", url);
	request.setRequestHeader('User-Agent','SpiderControl/1.0 (iniNet-Solutions GmbH)');
	request.addEventListener('load', function(event) {
		if (request.status >= 200 && request.status < 300)  {
		  adapter.log.debug("Set <"+addr+"> succssful to <" + value + ">");
		  adapter.log.debug(request.responseText);
		}
		else {
		  adapter.log.error("Error while set <"+addr+"> to <" + value + ">");
		  adapter.log.error(request.statusText, request.responseText);
		}
	});
}




